document.addEventListener("DOMContentLoaded", () => {
  ...
  const modelGroups = {
   ...
   "Transit (Alle)": /^Transit\s/i,
    ...
    const slimMarke = new SlimSelect({
      select: '#marke',
      closeOnSelect: true,
    ...
    // SlimSelect für Modell
    slimModell = new SlimSelect({
      select: '#modell',
      closeOnSelect: false,
      ...
      events: {
        afterChange: (newSelected) => {
          ...
          selectedValues.forEach(selectedValue => {
            ...
            if (regex && selectedBrand && modelData[selectedBrand]) {
              modelData[selectedBrand].forEach(model => {
                if (regex.test(model)) {
                  allValuesToSelect.add(model);
                }
              });
            } else {
    ...
    brandDropdown.addEventListener("change", function () {
      ...
      if (!selectedBrand || !modelData[selectedBrand]) {
        ...
        const modelList = modelData[selectedBrand] || [];
  
        modelList.forEach(model => {
          newOptions.push({ text: model, value: model });
        });
  
        Object.keys(modelGroups).forEach(groupName => {
          const regex = modelGroups[groupName];
          const hasMatch = modelList.some(model => regex.test(model));
          if (hasMatch) {
            newOptions.push({ text: groupName, value: groupName });
          }
        });
      }
  ...
  // SlimSelect für HU gültig
  new SlimSelect({
    select: '#hu-gueltig',
    placeholder: 'HU mind. gültig',
    allowDeselect: true,
    showSearch: false
  });
  ...
  // SlimSelect für Fahrzeughalter
  new SlimSelect({
    select: '#fahrzeughalter',
    placeholder: 'Fahrzeughalter',
    allowDeselect: true,
    showSearch: false
  });
  ...
  new SlimSelect({
    select: '#tueren',
    closeOnSelect: true,
    placeholder: 'Anzahl der Türen wählen',
    allowDeselect: true,
    showSearch: false
  });
  ...
  new SlimSelect({
    select: '#fahrzeugart',
    closeOnSelect: true,
    placeholder: 'Fahrzeugart wählen',
    allowDeselect: true,
    showSearch: false
  });
  ...
  new SlimSelect({
    select: '#halter',
    closeOnSelect: true,
    placeholder: 'Anzahl der Halter wählen',
    allowDeselect: true,
    showSearch: false
  });
  ...
  function toggleCustomUmkreis(value) {
  ...
  function toggleCustomSchadstoff(value) {
