...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!editId) {
    // Kein Edit-Modus -> normales "Neues Inserat anlegen"
    return;
  }

  try {
    const res = await fetch(`/api/inserat-edit/${encodeURIComponent(editId)}`, {
      credentials: "include"
    });
    const data = await res.json();
    if (!res.ok || !data.ok) {
      console.error("Fehler beim Laden des Inserats:", data.error);
      return;
    }

    const inserat = data.inserat;

    // ==== Beispiele – IDs musst du an dein Formular anpassen ====
    const titelInput = document.querySelector("#titel");
    const preisInput = document.querySelector("#preis");

    if (titelInput) titelInput.value = inserat.titel || "";
    if (preisInput) preisInput.value = inserat.preis || "";

    // TODO: hier Schritt für Schritt ALLE relevanten Felder befüllen
    // (EZ, km, Kraftstoff, Ausstattung, usw.)

  } catch (err) {
    console.error("Fehler beim Edit-Load:", err);
  }
});
