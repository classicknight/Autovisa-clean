document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("/getNutzerInfo", { credentials: "include" })
    ...
    .then(data => {
      ...
      // Navbar-Anzeige aktualisieren
      if (display) {
        display.innerHTML = isLoggedIn
          ? `<i class="fas fa-user-check"></i> Eingeloggt als: ${data.name || "Nutzer"}`
          : `<i class="fas fa-user-slash"></i> Nicht eingeloggt`;
      }

      // Navigation für Gespeicherte Autos / Meine Autos
    ...
    .catch(() => {
      const display = document.getElementById("auth-display");
      if (display) display.innerHTML = `<i class="fas fa-user-slash"></i> Fehler beim Laden`;
    });
